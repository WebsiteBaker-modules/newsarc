...
// $Id: save_group.php 399 2006-12-24 07:50:44Z Ruebenwurzel $
...
if($admin->get_post('title') == '') {
	$admin->print_error($MESSAGE['GENERIC']['FILL_IN_ALL'], WB_URL.'/modules/newsarc/modify_group.php?page_id='.$page_id.'&section_id='.$section_id.'&group_id='.$group_id);
} else {
...
// Update row
$database->query("UPDATE ".TABLE_PREFIX."mod_newsarc_groups SET title = '$title', active = '$active' WHERE group_id = '$group_id'");
...
if(isset($_FILES['image']['tmp_name']) AND $_FILES['image']['tmp_name'] != '') {
	...
	$query_settings = $database->query("SELECT resize FROM ".TABLE_PREFIX."mod_newsarc_settings WHERE section_id = '$section_id'");
...
if($database->is_error()) {
	$admin->print_error($database->get_error(), WB_URL.'/modules/newsarc/modify_group.php?page_id='.$page_id.'&section_id='.$section_id.'&group_id='.$group_id);
} else {
